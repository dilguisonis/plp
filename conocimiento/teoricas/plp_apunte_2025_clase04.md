...
1. Dar una derivaci´on de ⊢ $P \Rightarrow P$   
2. Dar una derivaci´on de ⊢ $P \Rightarrow Q \Rightarrow ( Q \land P )$   
...
2. Dar una derivaci´on de ⊢ $\cdot \left( P \lor P \right) \Rightarrow P .$   
...
2. Dar una derivaci´on de $\mathrm { ~ - ~ } \neg ( P \land \neg P ) .$   
3. Dar una derivaci´on de $P \lor Q \vdash \neg ( \neg P \land \neg Q ) .$

# Deducci´on natural intuicionista (NJ) — reglas completas

<table><tr><td>Introducción</td><td>ax ,T├T Eliminación</td></tr><tr><td>&gt;</td><td>「ττ&gt;σ 「ττ&gt;q 0 Ai Ne1 「ττ&gt;σ</td></tr><tr><td></td><td>「,τ├σ 「ττ⇒σ「├τ →i</td></tr><tr><td></td><td>e 「ττ→σ 「T9 「├T To 「├τνσ 「,τ├ρ 「,σ├</td></tr><tr><td></td><td>Vi Vi Ve ttvσ ├TVσ 「├p 「├⊥ TT e</td></tr></table>
...
<table><tr><td></td><td>「T」」τ ax 「,T├T 「├T 1 e Introducci6n Eliminaci6n</td></tr><tr><td>&gt;</td><td>「├τ「├σ 「├τ&gt;σ 「├τ&gt;σ Ni Ne1 e2 「├T&gt;σ 「-T 「-g 「,T├σ 「├T→σ 「-T</td></tr><tr><td>→</td><td>→i e 「├T→σ 「-σ</td></tr><tr><td>V</td><td>「├T 「-σ Vi Vi2 rtTVo 「├TVσ 「-p</td></tr><tr><td>上</td><td>「├」 Le 「-T</td></tr></table>
...
$$
\begin{array} { r l } { v \vdash P } & { { } { \mathrm { ~ s i ~ y ~ s \cap | o ~ s i ~ } } \quad v ( P ) = \mathsf { V } } \\ { v \vdash \tau \land \sigma } & { { } { \mathrm { ~ s i ~ y ~ s \cap | o ~ s i ~ } } \quad v \vdash \tau { \mathrm { ~ y ~ } } v \vdash \sigma } \\ { \vdash \tau \Rightarrow \sigma } & { { } { \mathrm { ~ s i ~ y ~ s \cap | o ~ s i ~ } } \quad v \vdash \tau \circ v \vdash \sigma } \\ { v \vdash \tau \lor \sigma } & { { } { \mathrm { ~ s i ~ y ~ s \cap | o ~ s i ~ } } \quad v \vdash \tau \circ v \vdash \sigma } \\ { v \vdash \bot } & { { } { \mathrm { ~ n u n c a ~ v a l e } } } \\ { v \vdash \tau } & { { } { \mathrm { ~ s i ~ y ~ s \cap | o ~ s i ~ } } \quad v \vdash \tau } \end{array}
$$
...
1. Un contexto Γ determina una variable $P \in { \mathcal { P } }$ si vale que $P \in \Gamma \circ { \mathsf { q u e } } \lnot P \in \Gamma .$
...
Sea $\rho = ( \sigma _ { 1 } \land \dotsc \land \sigma _ { n } ) \Rightarrow \tau$ . Sabemos que ⊨ $\rho$ . ¿Por qu´e? Alcanza con probar que ⊢ $\rho$ es derivable en NK. ¿Por qu´e?
