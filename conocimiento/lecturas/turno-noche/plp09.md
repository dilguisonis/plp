...
1. Un conjunto de s´ımbolos de funci´on $\mathcal { F } = \{ \mathbf { f } , \mathbf { g } , \mathbf { h } , \mathbf { \Omega } , \mathbf { \Omega } \} .$ Cada s´ımbolo de funci´on tiene asociada una aridad $( \geq 0 )$ . 2. Un conjunto de s´ımbolos de predicado $\mathcal { P } = \{ \mathrm { ~  ~ \Omega ~ } , \mathrm { ~  ~ \Omega ~ } , \dots \} .$ Cada s´ımbolo de predicado tiene asociada una aridad $( \geq 0 )$
...
$\begin{array} { l l l l l l } { \sigma } & { : : = } & { \mathsf { P } ( t _ { 1 } , \ldots , t _ { n } ) } \\ & { \mid } & { \enspace \downarrow } \\ & { \mid } & { \sigma \Rightarrow \sigma } \\ & { \mid } & { \sigma \land \sigma } \\ & { \mid } & { \sigma \lor \sigma } \\ & { \mid } & { \lnot \sigma } \\ & { \mid } & { \forall \ l X . \sigma } \\ & { \mid } & { \exists X . \sigma } \end{array}$ f´ormula at´omica contradicci´on implicaci´on conjunci´on disyunci´on negaci´on cuantificaci´on universal cuantificaci´on existencial
...
$\begin{array} { r l } & { \quad \quad \frac { \alpha , \mathbf { P } ( W , W ) , \mathbf { Q } ( X ) \vdash \mathbf { P } ( W , W ) } { \sigma , \mathbf { P } ( W , W ) \vdash \mathbf { Q } ( X ) \Rightarrow \mathbf { P } ( W , W ) } \quad \quad } \\ & { \quad \frac { \sigma , \mathbf { P } ( W , W ) \vdash \mathbf { Q } ( X ) \Rightarrow \mathbf { P } ( W , W ) } { \tau , \mathbf { P } ( W , W ) \vdash \exists Z . ( \mathbf { Q } ( X ) \Rightarrow \mathbf { P } ( W , Z ) ) } \stackrel { \exists } { = } } \\ & { \quad \frac { \mathbf { \ P } ( W , W ) \vdash \exists Y . \exists Z . ( \mathbf { Q } ( X ) \Rightarrow \mathbf { P } ( W , Z ) ) } { \mathbf { \ P } ( W , W ) \vdash \exists Y . \exists Z . ( \mathbf { Q } ( X ) \Rightarrow \mathbf { P } ( Y , Z ) ) } } \\ & { \quad \quad \quad \quad \Big | \vdash \exists Y . \exists Z . ( \mathbf { Q } ( X ) \Rightarrow \mathbf { P } ( Y , Z ) ) } \end{array}$ i ax ∃i σ ⊢ σ σ, P e ∃X W . P(X W ,
...
Bajo esta estructura, la f´ormula $\forall X . X = X + X$ es verdadera.
...
$\begin{array} { r l } { 1 . } & { \forall X . X = X } \\ { 2 . } & { \forall X . \mathsf { P } ( X ) \Rightarrow \forall X . \mathsf { P } ( \mathsf { f } ( X ) ) } \\ { 3 . } & { \forall X . \neg \mathsf { P } ( X ) \land \exists X . \mathsf { P } ( X ) \qquad \mathrm { i } } \\ { 4 . } & { \forall X . \exists Y . \mathsf { P } ( X , Y ) \Rightarrow \exists Y . \forall X . \mathsf { P } ( X , Y } \\ { 5 . } & { \forall X . ( \mathsf { P } ( X ) \Rightarrow \sigma ) \Rightarrow ( \exists X . \mathsf { P } ( X ) ) \Rightarrow } \end{array}$ satisfactible e inv´alida v´alida (∴ satisfactible) nsatisfactible (∴ inv´alida) ) satisfactible e inv´alida con $X \notin \mathsf { f v } ( \sigma )$ v´alida
...
Usar el hecho de que si $\mathsf { s } ( X ) = t$ entonces ${ \pmb { \mathsf { S } } } ( s { \pmb { \mathscr { X } } } : = t { \pmb { \gamma } } ) = { \pmb { \mathsf { S } } } ( s ) .$
