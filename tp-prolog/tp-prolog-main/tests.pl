...
test(pintadasValidas_ej4, [nondet]) :-
    L = [_, _, _, _, _],
    soluciones(L,
        pintadasValidas(r([2,1], L)),
...
test(pintadasValidas_ej4, [nondet]) :-
    L = [_, _, _, _, _],
    soluciones(L,
        pintadasValidas(r([1,1,1], L)),
        [[x,o,x,o,x]]).

test(resolverNaive_ej5, [nondet]) :-
    nn(1, NN), NN=nono(M, _),
    findall(M, resolverNaive(NN), MS),
    length(MS, N),
    assertion(N > 0).

test(resolverNaive_ej5, [nondet]) :-
    armarNono([[1],[1],[1]], [[1],[1],[1]], NN), NN=nono(M, _),
    findall(M, resolverNaive(NN), MS),
    assertion(term_variables(MS, [])).

test(pintarObligatorias_ej6, [nondet]) :-
    L = [_, _, _, _, _],
    soluciones(L,
        pintarObligatorias(r([2,1], L)),
        [[_,x,_,_,_]]).

test(pintarObligatorias_ej6, [nondet]) :-
    L = [_, _, _, _, _],
    soluciones(L,
        pintarObligatorias(r([4], L)),
        [[_,x,x,x,_]]).
...
test(deducir1Pasada_ej7, [nondet]) :-
    nn(0, NN),
    cantidadVariablesLibres(NN, N1),
    deducir1Pasada(NN),
    cantidadVariablesLibres(NN, N2),
    assertion(N2 < N1).

test(deducir1Pasada_ej7, [nondet]) :-
    armarNono([[2],[3],[2]], [[2],[3],[2]], NN),
    cantidadVariablesLibres(NN, N1),
    deducir1Pasada(NN),
    cantidadVariablesLibres(NN, N2),
    assertion(N2 < N1).
...
test(restriccionConMenosLibres_ej8, [nondet]) :-
    soluciones(R,
        restriccionConMenosLibres(
            nono(_, [r([1], [x,x,x]),
                     r([2], [A,B]),
...
test(resolverDeduciendo_ej9, [nondet]) :-
    nn(0, NN), NN=nono(M, _),
    findall(M, resolverDeduciendo(NN), MS),
    length(MS, N),
    assertion(N == 1).
...
test(solucionUnica_ej10, [nondet]) :-
    nn(1, NN),
    (solucionUnica(NN) -> true ; true).
